...
public class AWS {
    ...
    String bucketName= "ass1bucket1name1";

    String ManagerToLocalAppQueueName= "ManagerToLocalAppQueue"; //unique for each app
    String AppsToManagerQueueName= "AppsToManagerQueue"; //same queue for all apps
    String ManagerToWorkersQueueName= "ManagerToWorkersQueue"; //same queue for all workers
    String WorkersToManagerQueueName= "WorkersToManagerQueue"; //same queue for all workers
    ...
    //Ec2
    public String createInstance(String userData, String ami, String tagName){
        ...
        try {
            ...
            System.out.println("[DEBUG] Tag added successfully to instance: " + instanceId + " and tag: " + tagName);
    ...
    //Ec2
    public void terminateAllWorkersInstances(){
        String nextToken = null;
        try {
            do {
                DescribeInstancesRequest request = DescribeInstancesRequest.builder().nextToken(nextToken).build();
                        ...
                        if(instance.state().name().equals(InstanceStateName.RUNNING) ||
                            ...
                            for (Tag tag : instance.tags()) {
                                if (tag.key().equals("Name") && tag.value().equals(workerTag)) {
                                    ...
                                    System.out.println("[DEBUG] Worker instance terminated successfully: " + instance.instanceId());
                                }
                            }
                        }
                    }
            ...
            System.err.println("[ERROR] in terminating all workers instances: "+e.awsErrorDetails().errorMessage());
            System.exit(1);
        }
    }
    ...
    // S3
    // this method attempts to create an S3 bucket with the specified name and region.
    //If the bucket already exists, the method will not throw an exception, and if it doesn't exist,
    ...
    public void deleteAllObjectsFromBucket(String bucketName) {
        try {
            ...
            //S3 buckets can contain a large number of objects, and the result of listing objects might be paginated
            //to improve performance and reduce the amount of data transferred in a single response
            do {
    ...
    //s3
    public boolean checkIfFileExistsInS3(String bucketName, String key){
        try {
            ...
            return true; // If no exception is thrown, the file exists
        } catch (S3Exception e) { // An exception is thrown if the file doesn't exist or if there's an error
            return false;  
        }
    }
    ...
    public void createQueue(String queueName){
        try{
            ...
            //for a message to arrive in the queue before returning an empty response if no messages are available.
            //By setting it to 20 seconds, it means that if the queue is empty,
            //the ReceiveMessage operation will wait for up to 20 seconds for a message to arrive before returning an empty response.
    ...
    public Message getOneMessageFromQueue(String queueName){
        try{   
            ...
            //if the queue is empty, the ReceiveMessage operation will wait for up to 20 seconds
            //for a message to arrive before returning an empty response.
            if(messages.size() > 0){
            ...
            System.err.println("[ERROR] retrieving one message from SQS: "+ queueName + e.awsErrorDetails().errorMessage());
            return null;
        }
    }
